...
function getEndpointWireOffset(wire, side, direction) {
  ...
  // Separacion compacta para varios cables en un mismo borne: visibles, pero mas juntos.
  const spacing = Math.max(3, maxStroke * 0.75);
...
function drawWires() {
  ...
  state.wires.forEach((wire, wireIndex) => {
    ...
    hitPath.style.pointerEvents = (state.mode === 'edit-wire' || state.mode === 'delete-wire' || state.multiSelectMode || state.selectedWireId === wire.id) ? 'stroke' : 'none';
